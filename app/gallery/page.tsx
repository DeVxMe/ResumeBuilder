...
function ResumeCard({ resume }: { resume: any }) {
  ...
  return (
    <Card className="group hover:shadow-lg transition-all duration-300 overflow-hidden border border-gray-200 shadow-md bg-white">
          ...
          <div className="text-center">
            ...
            <div className="space-y-1">
              {topSkills.map((skill, index) => (
                <div key={index} className="text-xs bg-gray-200 rounded px-2 py-1 inline-block mr-1 text-gray-700">
                  {skill.name}
                </div>
              ))}
            </div>
      ...
      <CardHeader className="pb-2">
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <CardTitle className="font-heading text-base truncate text-gray-900">{resume.full_name}</CardTitle>
            {currentJob && (
              <CardDescription className="text-sm truncate text-gray-700">
      ...
      <CardContent className="pt-0">
        ...
        <Button className="w-full bg-indigo-600 hover:bg-indigo-700 text-white" size="sm" asChild>
